...
func main() {
	data, err := os.ReadFile("test-spdx.intoto.json")
	bundle, err := signAttestation(context.Background(), data)
	if err != nil {
		logrus.Fatal(err)
	}
	...
	if err != nil {
		logrus.Fatal(fmt.Errorf("encoding attestation: %w", err))
	}

	fmt.Println(string(bundleJSON))
}

func signAttestation(ctx context.Context, attData []byte) (*protobundle.Bundle, error) {
	...
	// entry, err = cosign.TLogUploadInTotoAttestation(ctx, rekorClient, dsseWrapped, rekorBytes)
	if err != nil {
		return nil, err
	}
